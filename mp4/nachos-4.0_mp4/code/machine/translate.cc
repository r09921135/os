...
Machine::Translate(int virtAddr, int* physAddr, int size, bool writing)
{
    ...
    int victim;  // the page that needs to be swapped out
...
// from the virtual address
	...
	if (vpn >= pageTableSize) {
		...
		bcopy(buffer, &mainMemory[j*PageSize], PageSize);        // save data into physical memory
            ...
            else{                           // there is no free frame, page replacement needs to be invoked 
		char *buffer1;
		char *buffer2;
    ...
    if (writing)
	entry->dirty = TRUE;
    *physAddr = pageFrame * PageSize + offset;   // final physical memory address
